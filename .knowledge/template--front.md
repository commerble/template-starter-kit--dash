...
一部のテンプレートはCommerble内部で他のテンプレートと文字列結合されます。このテンプレートを共有テンプレートと呼称し、大抵のテナントでは以下のテンプレートが共有テンプレートとして設定されています。

* ModdSharedViewStart
* ModdSharedFunctions
* ModdSharedHelpers

※ ModdShared接頭辞があるからと必ずしも共有テンプレートであるとは限りません。 本リポジトリでは`sync.js`の`sharedTemplates`設定で確認できます。

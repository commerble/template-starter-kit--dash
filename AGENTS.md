...
- `.vscode/mcp.json` で `envFile` が設定されている運用では、まず MCP サーバー経由で環境変数を参照する。
- MCP ツールが権限不足や接続不良で使えない場合、AIエージェントは勝手に別手段へフォールバックしない。作業を停止し、ユーザへエスカレーションして方針確認を行う。
- 上記の状態で `.env` を直接読み取る操作は、ユーザの明示的な許可がある場合に限定する。
